...
| 최근 제출 | 23회차 `cand_asof_f.zip` -> 1032.3182 (−8.55, §19) |
| 대기 후보 | `cand_asof_x.zip` (D×맥락, 빌드·검증 완료, §20) |
| 남은 제출 | **1장** |

### 🚩 선결 두 가지 — 답이 곧 행동이다

**(1) 리더보드가 최고점 유지인가, 마지막 제출 확정인가.**
최고점 유지면 1040.8656 이 남는다. 마지막 제출 확정이면 지금 우리 점수는
**1032.3182** 이고, 남은 1장은 `cand_asof.zip` 재제출에 묶인다.

**(2) 그 1장을 X 에 걸 것인가.**
X 는 §19-b 게이트(min ≥ 1.01, 노이즈 5배)를 **통과하지 못했다** (1.0049, 3.4~3.8배).
다만 증거의 질은 F 와 다르다 — 아래 §20 표 참조. **자율 세션에서는 제출하지
않았다.**
...
### 24회차 후보 (§20) — D×맥락 `cand_asof_x.zip`
...
| | 2022 | 2023 | 2024 |
|---|---:|---:|---:|
| 시드(42,43) | 1.0050 | 1.0202 | **1.0314** |
| 시드(44,45) | 1.0053 | — | 1.0242 |
| 마진/시드노이즈 | 3.8배 | — | 3.4배 |

생산 경로 2024 홀드아웃 **899.4 → 924.4 (배수 1.0278)** — 캐시 경로 두 시드쌍
평균과 소수점까지 일치. 아핀 `alpha` 1.090000 `center` 0.584040 (클린).
verify 4단계 통과(추론 5.8초), 실제 test.csv 손계산 대조 최대차 0.

**기대 1063.5~1066.1** (전이 78~87%), **하한 1045** (min gain 기준).
