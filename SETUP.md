...
**아핀도 클린이라 규정 4) 회색지대를 완전히 벗어난 상태에서 나온 점수다.**

### 남은 후보

| 파일 | 상태 |
|---|---|
| `submissions/cand_asof.zip` | ✅ 제출 완료 — **1040.8656** |
| `submissions/cand_nest_clean.zip` | 미제출. 954.3 기대. 챔피언+클린아핀 (§16) — 이제 열위라 쓸 이유 없음 |
| `submissions/cand_caafe*.zip` | 제출 완료 (952.4983 / 924.3786) |

### 다음에 할 것

**아핀은 건드리지 말 것** — 적률을 풀면 최적이 1041.1~1049.5 이고 현 손실이
0.2~8.6 뿐이다. `alpha` 1.09 가 최적 구간(1.020~1.160) 한가운데에 있다.

가치가 남은 곳은 **AS-OF 축의 확장**이다. 전이가 확인된 유일한 축이다. 후보:

1. **`prev1/3/5 게임 rate` vs `cur_rate` 비교.** 지금까지 CAAFE 는
   `prev1 - 통산rate` 를 썼는데 이는 척도가 다른 두 값을 뺀 것이었다. 이제
   `cur_succ`(2025 시즌내)가 있으므로 **같은 시즌 안에서 최근 경기 vs 시즌누적**을
   비교할 수 있다 — 질적으로 다른 비교다.
2. **E 변형(`cur - prior`)** 도 3/3 을 통과했고 min gain 이 D 보다 근소하게 높다
   (1.0332 vs 1.0322). D 와 조합했을 때 가산인지 확인할 가치가 있다.
3. 타자 쪽 `cur_*` 를 더 파기 (지금은 2개만 넣었다).

**판정은 반드시 `min(폴드별 배수)` 로 한다** — 아래 판정 규칙 참조.
