...
## 지금 상태 (2026-08-22 세션 종료)

| | |
|---|---|
| 최고 실측 LB | 🚩 **1076.2214381865** (48회차, `cand_mir.zip`) |
| 가중 9개 | `[0.133501, 0.231229, 0.234493, 0.350615, 0.703699, 0.903233, 0.647384, 1.984998, 2.105]` |
| 이력 | 1071.8146(36) → 1073.8237(39) → 1074.8798(40) → 1075.4602(41) → 1075.9242(45) → **1076.2214(48)** |
| 제출 예산 | 하루 5회 |
| 리더보드 | **약 200위.** 위쪽이 1080~1200 밴드 (전이 채널 사용도 분포로 설명됨) |

### 🔴 다음 세션에서 **가장 먼저** 할 일 — 시드 앙상블 마무리

```
목표   cand_s30 = cand_mir 의 시드를 3 -> 30 으로.  **+1.78 점 (수학적 보장)**
근거   rho = cov(μ,y)/sqrt(var(μ)+σ²/k) 는 k 에 단조 증가.  실패 불가
실측   σ²/var(p) = 0.0041(2022) / 0.0048(2023) / 0.0076(2024)
```

**이미 나와 있는 것** — `submissions/cand_s5.zip`(k=5, +0.6), `cand_s10.zip`(k=10, +1.19),
`cand_s15.zip`(k=15, +1.4, 저장됐다면). 관문만 돌리면 바로 제출 가능하다.

**이어서 하려면**:

```powershell
.env_submit\Scripts\python.exe -u expuild_seeds.py --seeds 30 --save-every 5
```
> ⚠️ 처음부터 다시 학습한다 (모델을 메모리에만 들고 있다가 5개마다 저장하는 구조).
> 시드 1개당 약 200초. 30개면 약 100분. **VS Code 를 닫으면 죽는다** (§9).
> 중간에 끊길 것 같으면 `--save-every 5` 를 유지해 부분 산출물을 확보할 것.

### 대기 중인 후보 (전부 관문 4개 통과)

| 파일 | sha256 | 근거 | 자격 |
|---|---|---|---|
| **`cand_s15.zip`** | `9da14385ff6f7634` | **시드 15 — 관문 4개 통과** | 1 (보장, **+1.40**) |
| `cand_s10.zip` | — | 시드 10 (관문 미실행) | 1 (보장, +1.19) |
| `cand_q1.zip` | `a72201b7885f8934` | 미측정 4dof **직교** 방향 | 2 (LB 연장) |

`cand_q1` 해석 규칙 — `> mir` 이면 3점 해법, `≈ mir` 이면 다음 직교 방향,
**`< mir` 이면 뒤집는다** (43→48회차가 정확히 그 경로였다: −1.36 → +0.2973).
