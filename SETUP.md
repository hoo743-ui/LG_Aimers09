...
| | |
|---|---|
| 최고 실측 LB | 🚩 **1044.7656** (24회차, `cand_asof_x.zip`) |
| 종전 | 1040.8656 (22회차) / 1032.3182 (23회차) |
| 남은 제출 | **확인 필요** — 08-13 기준 2장이었고 23·24회차로 둘 다 썼다 |

### 🚩 전이 법칙 (§21) — 이제 제출 전에 계산할 수 있다

세 회차(D·F·X)가 하나의 식에 맞는다. D 와 X 로 풀고 **F 로 검증**했다
(F 의 LB 예측 1032.18, 실측 1032.32 — 오차 0.14점).

```
실측 LB 이득 = 0.851 x (로컬 2024 홀드아웃 이득 - 2.34%)
```

| 축 | 로컬 2024 | 실측 배수 | 겉보기 전이율 |
|---|---:|---:|---:|
| D 현재상태 | +12.87% | 1.0897 | 70% |
| F 최근vs시즌 | +1.36% | 0.9918 | −60% |
| X D×맥락 | +2.78% | 1.0037 | 13% |

**손익분기는 로컬 2024 +2.34% 다.** 그보다 작은 이득은 LB 를 **내린다** —
F 의 실패는 제출 전에 산술로 알 수 있었다. `min gain` 게이트는 방향만 맞는
대리 지표였고, 이제 눈금이 있다.

**후보 평가 절차 (지금부터 이것만 쓴다)**
1. 생산 경로 2024 홀드아웃에서 Champion 대비 이득을 잰다
2. `2.34%` 를 뺀다. 음수면 **버린다** (제출하면 반드시 손해)
3. 남은 값 x `0.851` x 현재 LB 가 기대 이득이다

### 1100 까지

```
1044.7656 -> 1100  이려면  로컬 2024 이득 8.55% 필요
   D 가 12.87%,  X 가 2.78% 였다
```

**D 급 발견이 한 번 더 있어야 한다.** §20 에서 잰 여섯 축(창 2종, 축소 3종,
맥락) 중 손익분기를 넘은 것은 X 하나뿐이고 여유가 0.44%p 였다. 정제로는 못 간다.
