...
### 🚩 선결 — 리더보드가 최고점 유지인가, 마지막 제출 확정인가

**최고점 유지면** 1040.8656 이 남고 남은 1장은 쓸 데가 없다 (살아 있는 축이 없다).
**마지막 제출 확정이면** 지금 우리 점수는 1032.3182 이고, **남은 1장으로
...
### 23회차가 무엇이었나 (§18~19) — AS-OF 축은 D 하나로 끝났다

`prev{k}_game_rate − cur_rate`(F). 로컬 2024 +1.36%, **평가셋 −0.82% (전이 −60%)**.

세 갈래를 D 위에서 쟀고 E(`cur−prior`) 0.9679 · B(타자확장) 0.9713 은 즉시
기각됐다. F 만 3/3 · min gain 1.0026 으로 통과시켰는데 **그 1.0026 이 문제였다.**
