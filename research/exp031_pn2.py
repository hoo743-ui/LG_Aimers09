...
def main():
    E.start_experiment(EXP, "A-PN2", "python research/exp026_prune.py", "load")
    ft, sc = ba.ft, ba.sc
    tr = build_df()
    ...
    # PN2 — 직전 경기 투구 수를 **곱의 형태로** 준다.
    #
    # 원래 PN 시험은 log1p(pn1/pn3/pn5) 원시 3열뿐이었다. 그런데 원장의 분석은
    # 신호가 투구수 자체가 아니라 **prev_rate 의 신뢰도**에 있다고 말한다
    # (corr(prev1_rate, y): 1~5구 +0.0142  vs  91~200구 +0.0577).
    # 신뢰도는 `rate x f(count)` 라는 곱이고, 축평행 트리가 못 만드는 형태다.
    # X(+3.90)/H1(+5.16)이 통한 구조가 정확히 이것이다.
    pn = np.load(os.path.join(ROOT, "exp", "cache", "prevn.npy")).astype(np.float64)
    assert len(pn) == len(tr), f"길이 불일치 {len(pn)} vs {len(tr)}"
    pn[pn <= 0] = np.nan
    PN2 = []
    for j, w in enumerate((1, 3, 5)):
        lg = np.log1p(pn[:, j])
        tr[f"pn{w}"] = lg
        PN2.append(f"pn{w}")
        for lb in ("success", "middle"):
            c = f"asof_pitcher_prev{w}_game_{lb}_rate"
            if c in tr.columns:
                tr[f"pn{w}x{lb[:4]}"] = tr[c].astype(float) * lg
                PN2.append(f"pn{w}x{lb[:4]}")
    LEAN = FULL + PN2
    print(f"FULL {len(FULL)}열   +PN2 {len(LEAN)}열")
    ...
    out = {}
    base = None
    for nm, feats in (("FULL (현행)", FULL), ("+PN2 (투구수 곱)", LEAN)):
        preds = []
        for sd in SEEDS:
            E.beat(f"{nm} seed {sd}")
            t0 = time.time()
        ...
        print(f"{nm:<16}{len(feats):>5}{s1:>10.1f}{s2:>10.1f}{avg:>12.1f}"
              f"{avg - base:>+8.1f}")
        np.save(os.path.join(ROOT, "exp", f"pred24_{'full' if 'FULL' in nm else 'pn2'}.npy"),
                np.asarray(preds))

    gain = out["+PN2 (투구수 곱)"]["avg"] - out["FULL (현행)"]["avg"]
    print(f"\nPN2 이득 = {gain:+.1f}   (시드 잡음 ±7.2)")
    print("  참고 — 전부 NaN 인 8열 추가는 −13.5 였다 (열 추가 세금)")
    dec = "PROMISING" if gain > 3 else "REJECTED"
    E.set_hypothesis_status("A-PN2", "PROMISING" if dec == "PROMISING" else "CLOSED",
                            level=2, hypothesis="직전경기 투구수를 곱으로 (PN 재시험)",
                            result=round(gain, 2))
    E.finish_experiment(dict(
        experiment_id=EXP, hypothesis_id="A-PN2", type="A", level=2,
        started_at=E.read(E.CKPT)["start_time"], added=PN2,
    ...
    json.dump(out, open(os.path.join(ROOT, "exp", "exp031_pn2.json"), "w"),
