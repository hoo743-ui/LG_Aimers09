...
# 통산 비율 — 시즌내(cur_*)와 같은 물리 방향
T10 = dict(FULL, **{"asof_pitcher_middle_rate": -1, "asof_pitcher_reverse_rate": -1,
                    "asof_pitcher_ball_rate": -1, "asof_pitcher_strike_rate": 1,
                    "cur_str": 1})
# 직전 경기 창 — 같은 방향
T15 = dict(T10, **{"asof_pitcher_prev1_game_success_rate": 1,
...
def main():
    ...
    print("=" * 76)
    out, base = {}, None
    for name, spec in (("전체 5개 (기준)", FULL), ("통산까지 10개", T10),
                       ("직전경기까지 15개", T15)):
        E.beat(name)
        t0 = time.time()
    ...
    dec = "PROMISING" if (best != "전체 5개 (기준)" and gain > 2) else "REJECTED"
