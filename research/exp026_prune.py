...
def main():
    E.start_experiment(EXP, "P-DUP", "python research/exp026_prune.py", "load")
    ft, sc = ba.ft, ba.sc
    tr = build_df()
    ...
    LEAN = [c for c in FULL if c not in PRUNE]
    print(f"FULL {len(FULL)}열   LEAN {len(LEAN)}열   제거 {len(FULL)-len(LEAN)}열")
    ...
    out = {}
    base = None
    for nm, feats in (("FULL (현행)", FULL), ("LEAN (중복 제거)", LEAN)):
        preds = []
        for sd in SEEDS:
            E.beat(f"{nm} seed {sd}")
            t0 = time.time()
        ...
        print(f"{nm:<16}{len(feats):>5}{s1:>10.1f}{s2:>10.1f}{avg:>12.1f}"
              f"{avg - base:>+8.1f}")
        np.save(os.path.join(ROOT, "exp", f"pred24_{'full' if 'FULL' in nm else 'lean'}.npy"),
                np.asarray(preds))

    gain = out["LEAN (중복 제거)"]["avg"] - out["FULL (현행)"]["avg"]
    print(f"\n중복 제거 이득 = {gain:+.1f}   (시드 잡음 ±7.2)")
    print("  참고 — 전부 NaN 인 8열 추가는 −13.5 였다 (열 추가 세금)")
    dec = "PROMISING" if gain > 3 else "REJECTED"
    E.set_hypothesis_status("P-DUP", "PROMISING" if dec == "PROMISING" else "CLOSED",
                            level=2, hypothesis="증명된 중복 열 가지치기",
                            result=round(gain, 2))
    E.finish_experiment(dict(
        experiment_id=EXP, hypothesis_id="P-DUP", type="A", level=2,
        started_at=E.read(E.CKPT)["start_time"], pruned=PRUNE,
    ...
    json.dump(out, open(os.path.join(ROOT, "exp", "exp026_prune.json"), "w"),
